...
Once you open files, such as R scripts (click on the white square with the plus in green icon), another panel will open in the top left.
...
The simplest thing you can do with R is use it as a calculator:
...
R will show a `+` instead of a `>`. 
~~~
+
~~~

This means it's waiting for you to complete the command.  
If you want to cancel a command you can hit "Esc" and RStudio will give you back the ">" prompt.
...
Typing a <kbd>?</kbd> before the name of a command will open the help page
...
Notice also that objects can be reassigned:
...
Object names can contain letters, numbers, underscores and periods. They must start with a letter. They
...
`rm` to deletes objects you no longer need:
