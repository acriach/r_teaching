...
In the bottom left pane in RStudio, click the "Files" tab. To organise your project, create folders called `data`, `scripts`, `results`, `docs` by clicking the folder icon with the green plus sign. 
...
Open a script (click the white square with the cross in green below File). Save the script in your R project folder (click "File", then "Save as" and browse to the `script` folder in your project). You could call it "Lesson 1".
...
> **Challenge 2**
> Download the palmer penguins data from [here](https://raw.githubusercontent.com/acriach/r_teaching/main/data/penguins.csv).
>
...
R, and every package author, provide help files for functions. Either of these will load up a help page in RStudio:
...
Species is `chr` short for character which R will treat as a factor (in other words, discrete data). Bill_length_mm is `num` which stands for numeric. `int` means integer. Both `num` and `int` variables are treated as continuous (scale) data by R.
