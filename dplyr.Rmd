...
errors, and save you some typing. As an added bonus, you might
...
> **Challenge 1**
>
> Write a single command (which spans multiple lines and include pipes) that
...
<details>
  <summary>**Check your answer to Challenge 1**</summary>
...
If we want to do the above for each species, we'd have to repeat the code but we're trying to reduce the error prone repetitiveness of what can
be done with base R. In other words, using `filter()`, will only pass
...
> **Challenge 3**
>
> Write code that takes the table you created in Challenge 2, formats it using `gt()` function and exports it as a html file using `gtsave()`, saving it in your results folder. The table should look like the below.
...
<details>
  <summary>**Solution to Challenge 2 and 3**</summary>
...
Save the table
```{r, echo = TRUE, eval = FALSE}
gtsave(formatted_table, "results/penguin_summary_table.html")
```
...
```
Size is our name for the new column. You can read `~` as "then write". The last line of code can be read as "if none of the conditions are TRUE then write not Gentoo".
...
The filter function used `!is.na` read as "not is NA".
```{r}
penguins %>%
    group_by(species, sex) %>%
    filter(!is.na(sex))
...
An alternative is using `na.omit()`
```{r}
...
Be aware that`na.omit()` removes the entire row of data even if there is an NA value in a variable you are not using.
...
```{r}
ExampleData[ExampleData == 999] <- NA
```
