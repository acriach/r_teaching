...
If you have have not installed this package previously, please do so:
...
Remember, <kbd>`select()`</kbd> choose columns and <kbd>`filter()`</kbd> chooses rows 
...
Starting the code with <kbd>`species_body_means <-`</kbd> will assign the results as a new data frame called `species_body_means`. This <kbd>`species_body_means`</kbd> will then be listed under the environment tab so you can view it. Good for large data frames. 
...
```
 You can read `~` as "then write". The last line of code can be read as "if none of the conditions are TRUE then write is not Gentoo".
